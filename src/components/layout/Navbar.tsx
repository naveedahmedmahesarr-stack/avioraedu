  ...
  return (
    ...
    <header
      className={`fixed inset-x-0 top-0 z-50 transition-[background-color,box-shadow,padding] duration-500 ${
        solid ? "bg-navy-950/95 py-3 shadow-[0_10px_40px_-20px_rgba(0,0,0,.6)] lg:bg-navy-950/85 lg:backdrop-blur-xl" : "bg-transparent py-5"
      }`}
    >
      <nav aria-label="Main" className="container-x flex items-center justify-between gap-6">
            ...
            return (
              <li key={item.href}>
                <Link
                  ...
                  className={`group relative whitespace-nowrap rounded-full px-[0.45rem] py-2 text-[0.8rem] font-medium tracking-wide transition-colors 2xl:px-3 2xl:text-[0.82rem] ${
        ...
        <div className="flex items-center gap-3">
          <Link href="/contact#consultation" className="btn btn-gold hidden shrink-0 whitespace-nowrap !min-h-10 !px-5 !text-[0.8rem] !tracking-[0.05em] sm:inline-flex">
            Book a Consultation
          </Link>
          <button
            ref={toggleRef}
