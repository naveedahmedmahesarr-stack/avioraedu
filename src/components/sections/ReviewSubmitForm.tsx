...
export function ReviewSubmitForm() {
  const ui = useUi();
  const t = ui.reviewForm;
  const [status, setStatus] = useState<Status>("idle");
  const [errors, setErrors] = useState<Record<string, string>>({});
  const [message, setMessage] = useState("");
  const [rating, setRating] = useState(0);

  const localize = (fields: string[]) => Object.fromEntries(fields.map((k) => [k, t.errors[k] ?? ui.form.generic]));
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!parsed.success) {
      setErrors(localize([...new Set(parsed.error.issues.map((i) => String(i.path[0])))]));
      return;
    }
    setErrors({});
    ...
    try {
      ...
      if (!res.ok) {
        setErrors(localize(Object.keys(json.fieldErrors ?? {})));
        setMessage(res.status === 429 ? ui.form.tooMany : res.status === 422 ? ui.form.generic : ui.form.failed);
        setStatus("error");
        return;
      }
      setStatus("success");
    } catch {
      setMessage(ui.form.network);
      setStatus("error");
    }
  }
  ...
  if (status === "success") {
    return (
      <div role="status" className="rounded-3xl border border-success/30 bg-white p-8">
        ...
        <h3 className="mt-4 text-3xl text-navy-900">{t.successTitle}</h3>
        <p className="mt-2 text-stone">{t.successBody}</p>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={onSubmit} noValidate className="grid gap-5 rounded-3xl border border-navy-900/10 bg-white p-6 md:grid-cols-2 md:p-8">
      ...
      <div>
        <label htmlFor="rv-country" className="label">
          {t.country}
        </label>
