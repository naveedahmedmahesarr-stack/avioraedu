...
export function DestinationCard({ d, active, onSelect }: { d: Destination; active: boolean; onSelect: () => void }) {
  return (
    <button
      type="button"
      role="tab"
    ...
    >
      ...
      <span className="flex-1">
        ...
        {d.primary && <span className={`eyebrow mt-1 block !text-[0.58rem] ${active ? "text-gold-300" : "text-gold-600"}`}>Primary destination</span>}
      </span>
...
export function DestinationExplorer({ destinations }: { destinations: Destination[] }) {
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[340px_1fr] lg:gap-12">
      <div ref={listRef} role="tablist" aria-label="Destinations" aria-orientation="vertical" className="flex flex-col gap-3" onKeyDown={onKey}>
        {sorted.map((dest, i) => (
          <DestinationCard key={dest.id} d={dest} active={i === idx} onSelect={() => setIdx(i)} />
        ))}
      </div>
      ...
      <div role="tabpanel" id={`${id}-panel`} aria-live="polite" className="relative min-h-[520px] overflow-hidden rounded-[2rem] bg-navy-950 text-ivory">
        {d.heroImage ? (
          <Image key={d.heroImage} src={d.heroImage} alt={`${d.name} — study destination`} fill sizes="(min-width:1024px) 60vw, 100vw" className="object-cover opacity-50" />
        ) : (
        ...
        <div key={d.id} className="animate-fade-up relative flex h-full flex-col p-8 md:p-12">
          <div className="flex items-center gap-3">
            ...
            <span className="eyebrow text-gold-300">{d.primary ? "Primary destination" : "Selected destination"}</span>
          </div>
          ...
          <div className="mt-8 grid gap-8 md:grid-cols-2">
            <div>
              <h4 className="eyebrow text-navy-300">Study benefits</h4>
            ...
            <div>
              <h4 className="eyebrow text-navy-300">Cities</h4>
              <p className="mt-4 text-sm leading-relaxed text-ivory/85">{d.cities.join(" · ")}</p>
              {d.lifestyle.length > 0 && (
                <>
                  <h4 className="eyebrow mt-6 text-navy-300">Lifestyle</h4>
          ...
          <div className="mt-10 flex flex-wrap gap-3 pb-20 md:pb-28">
            <Link href={d.primary ? "/study-in-germany" : `/destinations/${d.slug}`} className="btn btn-gold">
              Explore {d.name} <Icon name="arrowRight" className="size-4" />
            </Link>
            <Link href={`/contact?destination=${encodeURIComponent(d.name)}#consultation`} className="btn btn-ghost-light">
              Ask about {d.name}
            </Link>
          </div>
        </div>
