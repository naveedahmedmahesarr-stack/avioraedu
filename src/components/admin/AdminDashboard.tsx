...
import { api, uploadFile, type StorageDriver } from "./upload";
import { checkFile } from "@/lib/media/rules";

type Item = Record<string, unknown> & { id?: string; published?: boolean };
const ORDER: CollectionName[] = ["submissions", "media", "settings", "reviews", "dreamStories", "destinations", "universities", "faqs", "team", "homepage"];

function MediaField({ field, value, onChange, storage }: { field: Field; value: string; onChange: (v: string) => void; storage: StorageDriver }) {
          ...
          <input
            ...
            onChange={async (e) => {
              ...
              try {
                ...
                onChange(await uploadFile(file, file.type, storage));
              } catch (x) {
...
function Editor({ name, item, onClose, onSaved, storage }: { name: CollectionName; item: Item; onClose: () => void; onSaved: () => void; storage: StorageDriver }) {
            ...
            return (
              <div key={f.key}>
                ...
                {f.type === "textarea" ? (
                  ...
                  <MediaField field={f} value={String(v ?? "")} onChange={(x) => set(f.key, x)} storage={storage} />
                ) : (
...
export function AdminDashboard({ storage }: { storage: StorageDriver }) {
