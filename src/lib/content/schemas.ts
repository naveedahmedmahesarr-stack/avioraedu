...
export const MEDIA_CATEGORIES = ["Germany Visits", "Student Life", "Success Stories", "Universities", "Office", "City Life"] as const;

/** Germany Insights — admin-uploaded photos and videos. */
export const mediaSchema = z.object({
  ...
  src: optionalUrl.refine((v) => v !== "", "Upload a file first"),
  /** Video poster (auto-generated from a frame at upload); ignored for images. */
  poster: optionalUrl,
  width: z.number().int().min(1).max(20000).default(1600),
  height: z.number().int().min(1).max(20000).default(900),
  /** Seconds, videos only. */
