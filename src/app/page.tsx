...
import { BrandShowcase3D } from "@/components/sections/BrandShowcase3D";
import { InsideAviora } from "@/components/sections/InsideAviora";
...
export default async function HomePage() {
  await connection();
  const [home, destinations, universities, stories, reviews, team, media] = await Promise.all([
    getHomepage(),
  ...
  return (
    <>
      ...
      <InsideAviora />
      <InsightsTeaser media={sortMedia(media).filter((m) => !m.id.startsWith("seed-"))} />
      <TrustSection />
